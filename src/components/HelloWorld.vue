<template>
	<div>
		<div :key="thread.slug" v-for="thread in threads" class="col-large push-top">
			<h1>{{ thread.title }}</h1>
			<div class="post-list">
				<div :key="postId" v-for="postId in thread.posts" class="post">
					<div class="user-info">
						<a href="profile.html#profile-details" class="user-name">{{ users[posts[postId].userId].name }}
						</a>
